...
int main(int argc, char* argv[])
{
	...
	//Crear superficies únicas para cada thread
	SDL_Surface** fractal_parts = new SDL_Surface * [nprocs];
	for (int i = 0; i < nprocs; i++)
		fractal_parts[i] = SDL_CreateRGBSurface(0, SCREEN_WIDTH / nprocs, SCREEN_HEIGHT, 
				32, 0, 0, 0, 0);
	...
	//Crear espacio de vectorización
	...
	while (running)
	{
		...
		if (progress > 0.9999999999999)
		{
			progress = 0;
			step = original_step;
#if LOOP_ONCE //Detener la ejecución cuando se genere la cantidad estándar de datos
			running = false;
#endif
		}
...
#pragma omp parallel num_threads (nprocs)
	...
	// write data to file
	std::ofstream file("../../../../../results_par_SIMD.csv", std::ios::out);
	for (auto& line : data)
		file << line << "\n";
	file.close();
...
void drawMandelbrotSIMD(int w, int h, double x_min, double y_min, double x_max, double y_max, SDL_Surface** target, complex *c, complex *z)
{
	...
	for (int i = 0; i < w; i++)
		for (int j = 0; j < h; j++)
		{
			...
			//Paralelizar la computación de iteraciones vectorizadas
#pragma omp simd
